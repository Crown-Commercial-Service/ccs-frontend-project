...
> - has access for publishing on npm
...
4. Pick a new version number according to the [versioning documentation](/docs/contributing/versioning.md) and apply it by running:
   ...
   ```shell
   npm version --no-git-tag-version --workspace ccs-frontend <NEW VERSION NUMBER>
   ```

   ...where `<NEW VERSION NUMBER>` is the literal number without a 'v' in front of it. This step will update [`ccs-frontend`'s `package.json`](/packages/ccs-frontend/package.json) and project [`package-lock.json`](/package-lock.json) files.
...
5. Create and check out a new branch (`release-[version]`)
   ...
   ```shell
   git switch -c "release-$(npm run version --silent --workspace ccs-frontend)"
   ```
...
6. Update the [`CHANGELOG.md`](/CHANGELOG.md) by:
   ...
   - if the changelog has headings from [pre-releases](/docs/releasing/publishing-a-pre-release.md#publish-a-new-version-of-ccs-frontend), regroup the content under those headings in a single block
   - saving your changes

7. Run `npm run build-release` to:

   - build CCS Frontend into [the package's `/dist`](/packages/ccs-frontend/dist) directory
   - commit the changes
...
4. Follw the steps [from the following section](#create-a-release-on-github) and create a GitHub release.

   This will automatically trigger a job to release the package to NPM with the `latest` tag
...
2. Sign in to npm (`npm login`), using the correct credentials
...
5. You will now be prompted to continue or cancel the release. Check the details and enter `y` to continue. If something does not look right, press `N` to cancel the release.
   ...
   This step will also automatically create a tag in Github which you can use to create a [Github release in the following section](#create-a-release-on-github).

6. Verify the presence of the release and its tag on [npm](https://www.npmjs.com/package/ccs-frontend?activeTab=versions)
...
7. Have a quick look at the diff of the new package with the previous one at:
   ...
   ```txt
   https://diff.intrinsic.com/ccs-frontend/<PREVIOUS_VERSION_NUMBER>/<RELEASED_VERSION_NUMBER>
   ```
