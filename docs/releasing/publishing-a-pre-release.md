# Before you publish a pre-release of CCS Frontend
...
6. Apply the new pre-release version number by running:
   ...
   ```shell
   npm version < PRE_RELEASE_VERSION_TYPE > --preid < PRE_RELEASE_IDENTIFIER > --no-git-tag-version --workspace ccs-frontend
   ```

   This step will update [`ccs-frontend`'s `package.json`](/packages/ccs-frontend/package.json) and project [`package-lock.json`](/package-lock.json) files.
...
9. Run `npm run build-release` to:
   ...
   - build CCS Frontend into [the package's `/dist`](/packages/ccs-frontend/dist) and [root `/dist`](/dist) directories
   - commit the changes
...
2. Sign in to npm (`npm login`), using the credentials for the ccs-patterns-and-tools npm user from Bitwarden.
...
5. You will now be prompted to continue or cancel the release. Check the details and enter `y` to continue. If something does not look right, press `N` to cancel the release.
   ...
   This step will also automatically create a tag in GitHub which you can use to create a [GitHub release in the following section](#if-publishing-a-beta-pre-release-create-a-release-on-github).

6. Verify the presence of the pre-release and its tag on [npm](https://www.npmjs.com/package/ccs-frontend?activeTab=versions)
