...
CURRENT_VERSION=$(npm run version --silent --workspace ccs-frontend)
...
# when looking for our `@ccs-frontend/xyz@*` packages
# pointing to the npm workspaces in our source
echo "✍️ Remove dev dependencies"
npm pkg delete devDependencies --workspace ccs-frontend

echo "✍️ Update package version"
npm version $VERSION --allow-same-version --no-git-tag-version --workspace ccs-frontend
git add packages/ccs-frontend/package.json
git add package-lock.json

echo "✍️ Force commit package"
git add --force packages/ccs-frontend/dist/
git commit --allow-empty -m "Release CCS Frontend 'v$VERSION' to '$BRANCH_NAME' for testing"

# Create a local branch containing the packages/ccs-frontend directory
echo "✨ Filter the branch to only the packages/ccs-frontend/ directory..."
git filter-branch --force --subdirectory-filter packages/ccs-frontend
