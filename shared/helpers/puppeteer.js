...
const { renderPreview } = require('@ccs-frontend/lib/components')
const { componentNameToClassName } = require('@ccs-frontend/lib/names')
  ...
  try {
    ...
    const error = await page.evaluate(
      async (selector, exportName, config) => {
        const namespace = await import('ccs-frontend')
  ...
  if (protocol === 'file:' && pathname.startsWith('/assets/')) {
    if (pathname.startsWith('/assets/fonts')) {
      return request.respond({
        body: readFileSync(join(paths.root, `node_modules/govuk-frontend/dist/govuk/${pathname}`)),
        contentType: mime.lookup(pathname) || 'text/plain'
      })
    } else {
      return request.respond({
        body: readFileSync(join(paths.package, `dist/ccs/${pathname}`)),
        contentType: mime.lookup(pathname) || 'text/plain'
      })
    }
  }
